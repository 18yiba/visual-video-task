...
已加入本地EEG断流保护：连续5秒无新样本（启动等待10秒）停止并显示警告；每秒采集状态和即时错误单独落盘。旧17/34组也可安装不切换协议的[小补丁与排查说明](docs/EEG_DISCONNECT_GUARD.zh-CN.md)。
...
采集保护的可选设备字段：`device.eeg_no_sample_timeout_sec`（默认5.0秒）、`device.eeg_startup_timeout_sec`（默认10.0秒）。未写入YAML时采用默认值；只监控本地样本到达，不判断阻抗或电池状态。外部BCIGo独立录制模式不具备此样本监控。
