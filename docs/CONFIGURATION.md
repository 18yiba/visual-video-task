...
```text
video_eeg/config/video_config.yaml       # formal: 17 sessions, 18 attention/session
video_eeg/config/video_demo_config.yaml  # demo-only short queue; participant rest threshold remains formal
video_eeg/config/session_manifest.csv    # fixed duration-balanced formal membership
...
默认写入 `data/video_question_complete_runs/<subject>/session_<NN>/session_state.json`。
