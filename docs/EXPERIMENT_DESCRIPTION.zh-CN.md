# Emotion EEG v1 实验说明

本实验在连续观看视频时记录 EEG，并在每段情绪视频后依次报告自身情绪感受（Valence）和唤醒程度（Arousal）。
每项为 1–9 整数、键盘选择、不限时。刺激类别只用于材料分组，不向被试显示；不存在正确答案或准确率。
普通视频后无评分。新协议共 45 组，每组正/中/负数量相等，净视频约 90 分钟，评分和休息另计。
视频、评分及休息期间 EEG 保持连续；S/Esc/J、长短休息及恢复机制沿用主框架并适配评分的完整提交。

详见 [完整协议](EMOTION_EEG_PROTOCOL.zh-CN.md)、[心理学依据及页面原文](EMOTION_RATING_RATIONALE.md)。

---

## 旧内容抽查协议说明（仅供历史数据解释）

以下不适用于 Emotion EEG v1 新采集。

# 视频观看与 EEG 采集实验说明

## 目的与任务

本范式在被试观看视频时连续记录脑电，并用少量视频内容抽查记录观看任务的行为表现。
不要求被试对情绪、愉悦度或唤醒度进行主观评分。抽查作答的正确性与脑电信号质量分别保存和解释。

## 材料与实验组织

视频母库共 7996 段；其中 47 段超过正式实验的 60 秒上限，保留原文件但不纳入正式 Session。
正式实验为 7949 段，固定分配到 34 个 Session，每组 233–235 段。分组由版本化 CSV 确定，
每位被试在 Session 内的播放顺序随机生成并保存。已完成的视频不因再次启动而重新分配。

2026-09-08 可取得的“完整版”题库有 7993 道题，覆盖正式视频 7946 段。
`5728.mp4`、`6722.mp4`、`6883.mp4` 暂无可用题。这三段仍可播放，但不作为抽查目标。
2779 道题为原复核通过题；5214 道为已生成且通过结构检查、尚未独立复核的题。
“有题”不代表已逐题核实视觉事实。本次审计核对了结构、文件覆盖和运行记录，未逐段复核题意。
可复现统计见 `video_eeg/config/complete_questions_20260908/coverage_audit.json`。

材料通过 [统一版本化 Release](https://github.com/18yiba/visual-video-task/releases/tag/materials-v1-20260910)
分包分发。安装环境后运行 `download_materials.bat` 下载至 `stimuli/videos` 并验证每段视频的 SHA-256。
固定分组、题库和材料清单必须使用同一版本；部署步骤见 [视频库下载说明](MATERIALS_DOWNLOAD.zh-CN.md)。

## 时间流程

1. 主试填写被试编号、Session 和窗口设置，展示总体实验说明。
2. 检查脑电设备连接及数据流，连接确认后开始记录。
3. 每段视频前显示注视点，默认 1.5 秒；视频播放至自然结束。
4. 如该视频被选中抽查，立即呈现与刚才视频对应的一道四选一题。
5. 视频或作答后短暂休息，默认 2 秒；空格可提前继续。
6. 连续净观看达到随机 30–45 分钟阈值后，在视频边界提示较长休息。
7. 全部视频及 18 次抽查完成后，Session 标记完成并保存数据。

净观看时间只累计提交完成视频的真实时长，不含注视点、加载、答题、休息和中断片段。
正式 Session 总耗时取决于材料时长、答题和休息，不能把净观看时间当作墙钟总时长。

## Alarm test：视频内容抽查

每个正式 Session 固定 18 次。程序将随机播放队列中有题的视频按顺序分成 18 段，
每段随机选择一个视频，保证目标不重复并分散在 Session 中。计划包含视频绑定和预计净时间。
正常不跳过视频时，问题对应立即前一段视频；S 跳过后，问题跟随该视频完整重播再出现。

抽查页提示：“请根据刚才的视频内容选择最符合的一项，答题不限时。”
按数字 1–4 或字母 A–D 作答，不限时，不显示正确答案反馈。回答后不论对错均完成一次抽查；
准确性与反应时另行记录。不会在 Session 末尾集中补问无关的前序视频。

## 中断、恢复与版本

S 中断当前视频，将其放回队列等待完整重播。Esc 保存并退出。
若在抽查页退出，视频和该次抽查不提交为完成，恢复后先完整重播该视频，再显示同一题。
自然 EOF 的视频 attempt 与“已提交完成的 Session 视频”是两个层次，分析时须结合 state 和答题状态。
题库 SHA-256、题目及随机计划写入断点。不同题库、不同抽查协议或次数的断点不能混用。

当前常用入口使用 `data/video_question_complete_runs/protocol_34sessions`；旧 2779 题正式入口的进度保留在
`data/video_question_runs`，可通过 `run_video_legacy_2779.bat` 完成旧 Session。

## 采集与数据契约

当前正式默认配置为 BrainCo SDK 直连、期望采样率 1000 Hz。实际通道、采样率等以设备和采集元数据为准。
运行前须确认设备接触、数据流稳定、视频显示及事件记录正常。代码不对采样率不符做静默重采样。
连续信号、事件、视频 attempt、回答和断点均保存在本地，参见操作文档的数据目录说明。

呈现事件沿用现有 `attention_task_on` marker 映射；`attention_response` 是本地行为事件。
事件和 CSV 中包含视频、attempt 和抽查 ID，可将回答关联到对应视频。
题目来源与复核状态保留在 `question_source`、`question_review_status` 字段中。

## 数据质量与行为表现分别解释

参考 [oi-eegqc 中文说明](https://github.com/Omni-Intel/oi-eegqc/blob/main/README.zh-CN.md)，
数据验收应先明确通道名称/配置、采样率、物理单位、视频片段边界与事件可信度，
再评估信号完整性、坏道、饱和、噪声及有效时长。答题正确率是行为指标，不应直接充当脑电质量分数。
该项目也要求明确单位；不能把未知的原始计数直接当作微伏。

本程序没有集成或自动执行 oi-eegqc，也不输出其等级、GQI 或自动合格结论。
如后续接入质控，需先根据设备/SDK文档及实测确定单位和换算，读取实际通道信息，
按事件切分片段，在副本上运行分析并独立保存质控报告；原始采集数据保留。

## 验证范围

软件测试与模拟 EEG 验证检查流程、日志、退出恢复和材料对应。
真实硬件的电极接触、实际采样率、信号质量及刺激至采集的物理延迟仍需主试在实验设备上验证。

## 当前 34 组与旧 17 组

7949 段正式视频总净时长约 50.30 小时。新分组每组 88.35–88.94 分钟，平均 88.76 分钟，
每组 233–235 段，五档视频各至少 46 段；全库正式视频范围约 5.107–59.976 秒。
每个原 Session 按五档时长均衡拆为两半，原 n 对应新 2n−1 和 2n。各组内部随机播放；
播放顺序和 18 个抽查目标在开始时保存，续跑不重新抽签。没有 90 分钟强制停止。
注视点、视频间隔、答题及休息不计入净观看，实际占用时间会更长。

当前权威分组为 `video_eeg/config/session_manifest_34.csv`。
旧 `session_manifest.csv` 保留原 17 组，不能用新文件覆盖旧文件。
旧完整版 17 组请用 `run_video_legacy_17.bat`，其数据仍在 `data/video_question_complete_runs/<subject>`。
新默认入口用 `data/video_question_complete_runs/protocol_34sessions/<subject>`，避免同编号读到旧状态。
不自动迁移已开始的旧 Session，不把旧完成进度强行当作新分组的完成进度。

`video_eeg/config/video_question_labels.csv` 是 7996 段视频的完整标签对照表；
包含 SHA-256、题干、选项、答案、复核状态、`session_34` 和 `legacy_session_17`。
Release 的 `material_metadata.zip` 也包含此表。题目经 `video_file` 文件名关联，未嵌入 MP4 画面或字幕。
原题库可能保留历史 `session_id`，当前分组以 34 组清单和标签表的 `session_34` 列为准。
详见 [34 组与标签审计](SESSION_34_AND_LABEL_AUDIT.zh-CN.md)。
