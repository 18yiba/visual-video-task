...
## 实验室 WinError 5 与 EEG 健康监测审计

依据两台实验室电脑事故反馈，检查保存、错误退出、断流与日志代码。本机通过保持目标文件读取句柄打开，复现 Path.replace 的 WinError 5；模拟连续100次空 EEG 读取未触发终止。未取得事故原始 traceback，不能定案。当前没有阻抗质量阈值或可靠的运行中断流超时，错误处理的二次写盘还可能阻断 EEG 导出。详见 [审计及现场排查说明](WINERROR5_AND_EEG_MONITORING_AUDIT.zh-CN.md)。本次未修改运行代码或原离线更新包，未修改采集数据。

## 2026-09-10 实验室硬盘离线最小更新

新增离线更新构建器与启动桥接脚本，生成约1.9 MiB小包，仅添加 `_video_eeg_34_update` 文件夹。沿用原电脑 Python 3.12、视频路径和设备配置；保留旧17组入口与数据，新34组记录独立存储。不打包环境、视频或被试数据，不调用在线安装器。检查不通过时停止新版，旧程序保持可用。

已验证带空格的模拟旧目录、7949段视频文件大小、34组抽查安排、设备配置继承及旧文件哈希不变；真实窗口完成一个视频内容题并导出模拟 EEG。真实实验室硬件待现场验证。详细步骤见 [离线更新说明](OFFLINE_LAB_UPDATE.zh-CN.md)，验证边界见 [测试记录](OFFLINE_UPDATE_VALIDATION.md)。
