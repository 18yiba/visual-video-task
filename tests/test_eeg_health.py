...
def test_fault_persisted_and_export_survives(mode,tmp_path):
    ...
    assert json.loads((tmp_path/'metadata.json').read_text())['termination_reason']=='eeg_background_error'
    assert 'eeg_acquisition_error' in [r['name'] for r in json.loads((tmp_path/'events.json').read_text())]
...
def test_external_recording_does_not_claim_stream_monitoring(tmp_path):
    ...
    assert json.loads((tmp_path/'metadata.json').read_text())['eeg_health_monitor']['enabled'] is False
