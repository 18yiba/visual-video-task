...
def test_ordinary_transaction_and_immediate_fatigue(monkeypatch,interrupted):
    ...
    def page(row,prefix,*args):
        ...
        return '1' if prefix=='alarm' else '7' if prefix=='fatigue' else 'j'
    r._choice_page=page
    ...
    if interrupted:
        ...
        assert row['liked']==1 and row['fatigue_rating']==7 and row['correct']
        assert row['fatigue_scale_min']==1 and row['fatigue_scale_max']==7
...
@pytest.mark.parametrize('answer',['1','7','num_7'])
def test_fatigue_rejects_binary_and_out_of_range_keys_and_waits_for_release(monkeypatch,answer):
    ...
    r.keyboard.getKeys.side_effect=[['1'],['f','j','0','8','9'],[answer]]
    monkeypatch.setattr(base,'core',SimpleNamespace(wait=lambda _:None))
    row={};keys=list('1234567')+[f'num_{i}' for i in range(1,8)]
    assert r._choice_page(row,'fatigue','Q',['anchors'],keys,'FATIGUE')==answer
    assert r.keyboard.getKeys.call_count==3
    assert row['fatigue_rating']==parse_rating(answer,7)
    assert events[-1][1]['fatigue_rating']==parse_rating(answer,7)
    assert events[0][1]['fatigue_scale_max']==7

def test_saved_binary_session_keeps_original_contract_but_new_session_uses_seven(tmp_path):
    import hashlib,json
    ...
    assert session_fatigue_scale({**cfg,'session_id':2},proto,600)==7
    state.manifest_hash=fatigue_contract_hash('materials',600,7);save_state_atomic(path,state)
    assert session_fatigue_scale(cfg,proto,600)==7

def test_binary_session_constructor_resume_preserves_answers_and_snapshot(tmp_path,monkeypatch):
    import copy
    ...
    original=make()
    original.state.manifest_hash=fatigue_contract_hash('materials',original.interval_sec,2)
    original.state.video_attempts=[dict(video_id='p',completed=False,ordinary_behavior=dict(fatigued=1,fatigue_key='j',completed=False))]
    ...
    resumed=make()
    assert resumed.fatigue_scale_max==2
    ...
    cfg['subject_id']='NEW'
    new=make()
    assert new.fatigue_scale_max==7
